    ...
    public partial class Racer
    {
        public class Replay
        {
            ...
            public void Export(string filename)
            {
                uint len_head = 0x27;
                uint len_meta = 0x30;
                uint len_def = frame_item_count * 0x9;
                uint len_body = frame_count * frame_data_size;
            ...
            private uint frame_item_count => data.Count() > 0 ? (uint)data[0].Data.Count() : 0;

            private uint frame_count => (uint)data.Count();

            private uint frame_data_size
            {
                get
                {
                    if (frame_item_count == 0)
                        return 0;
                    else
                    {
                        uint bytes = 0;
